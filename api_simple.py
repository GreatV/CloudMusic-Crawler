...
def get_music_lyric(song_id):
    url = 'http://music.163.com/api/song/lyric?os=pc&id={}&lv=-1&kv=-1&tv=-1'.format(
        song_id)

    try:
        r = requests.get(url, headers=headers, cookies=cookies)
        if 'lrc' in r.json() and 'lyric' in r.json()['lrc'] and r.json()[
                'lrc']['lyric'] is not None:
...
def get_music_comments(song_id, offset=0, total='fasle', limit=100):
    url = 'http://music.163.com/api/v1/resource/comments/R_SO_4_{}/'.format(
        song_id)
    payload = {
