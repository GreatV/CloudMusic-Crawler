...
def file_handler_temp(artistslist):
	with open('alltext.txt', 'w') as file_all:
		for artist_id in artistslist.values():
			with open('{}_alltext.txt'.format(artist_id), 'w') as fw:
				with open('{}.txt'.format(artist_id), 'r') as fr:
					print 'Reading No.{} musiclist...'.format(artist_id)
					for line in fr:
						line = line.strip()
						with open('{}.txt'.format(line)) as fi:
